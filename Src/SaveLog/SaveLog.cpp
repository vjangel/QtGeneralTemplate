...
#define QDATE_TIMS qPrintable(QDateTime::currentDateTime().toString("yyyy-MM-dd HH:mm:ss:zzz"))
...
#endif
{
    ...
    if (SaveLog::Instance()->getUseContext()) {
        ...
        if (line > 0) {
//            content = QString("[Line]: %1  [File]: %2  [Fun]: %3\n>>>%4").arg(line).arg(file).arg(function).arg(content);
//            content = QString("行号: %1  文件: %2  函数: %3\n%4").arg(line).arg(file).arg(function).arg(content);
            content = QString("[%1] [File]: %2  [Fun]: %3\n>>>%4").arg(QDATE_TIMS).arg(file).arg(function).arg(content);
        }
    }
#endif
